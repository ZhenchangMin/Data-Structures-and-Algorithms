...
**Queue** ADT represents a collection of items in which we could `add` and `remove` them

#### FIFO Queue

The first-in-first-out Queue, items are removed in the same order as they're added.
...
Deque(Double-Ended Queue), represents a sequence of items with a front and a back, supporting:
...
A Deque is a generalization of FIFO and LIFO Queue, and can implement both of them.
...
So Arraylist is fast for Stack, slow for FIFO Queue.
...
When we implement FIFO Queue, we can add at tail and remove at head, so it is fast.
